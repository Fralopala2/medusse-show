...
| \# MySQL Database for Users and Authentication  mysql:    image: mysql:8.0    container\_name: medusse\_mysql    restart: unless-stopped    environment:      \- MYSQL\_ROOT\_PASSWORD=medusse2025      \- MYSQL\_DATABASE=medusse\_db      \- MYSQL\_USER=medusse\_user      \- MYSQL\_PASSWORD=medusse2025    ports:      \- "13306:3306"    volumes:      \- ./mysql/data:/var/lib/mysql      \- ./mysql/init:/docker-entrypoint-initdb.d    networks:      \- medusse\_network    command: \--default-authentication-plugin=mysql\_native\_password |
| :---- |
...
| const cors \= require('cors');app.use(cors({  origin: \['http://localhost:4003', 'http://localhost:4001'\],  credentials: true,  methods: \['GET', 'POST', 'PUT', 'DELETE'\],  allowedHeaders: \['Content-Type', 'Authorization'\]}));Helmet para Seguridad de Headersconst helmet \= require('helmet');app.use(helmet({  contentSecurityPolicy: false, // Deshabilitado para desarrollo  crossOriginEmbedderPolicy: false})); |
| :---- |
