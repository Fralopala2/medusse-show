...
2. Tras `docker compose restart` o reiniciar Docker Desktop, **reinicia el simulador**:

   ```powershell
   powershell -ExecutionPolicy Bypass -File scripts\restart-simulator.ps1
   ```

3. En Grafana, usa rango **Last 15 minutes** y recarga.
4. `Test-NetConnection localhost -Port 1883` debe devolver `TcpTestSucceeded : True`.
